<?php
	...
	class UrlShortener
  {
  	...
  	public function generateUniqueCode($num)
  	{
  		$num = $num + 10000000;
  	...
  	public function validateUrlAndReturnCode($orignalURL)	{
  		...
  		if (!filter_var($orignalURL, FILTER_VALIDATE_URL)) {
  			...
  			if ($orignalURL->num_rows) {
  				$uniqueCode = $orignalURL->fetch_object()->code;
  				return $uniqueCode;
  			}
  			...
  			$secretCode = $this->generateCode($getIdOfRow);
  			$updateInDatabase = $this->db->query("UPDATE link SET code = '{$secretCode}' WHERE url = '{$orignalURL}'");
  	...
  	function generateLinkForShortURL($uniqueCode = '')
  	{
